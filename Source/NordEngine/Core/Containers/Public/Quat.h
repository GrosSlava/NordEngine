...
public:
	...
	*/
	FORCEINLINE FQuat(float InX, float InY, float InZ, float InW) noexcept : X(InX), Y(InY), Z(InZ), W(InW) { }
	/**
...
public:
	...
	*/
	FORCEINLINE FQuat operator-(const FQuat& Q) const noexcept { return FQuat(X - Q.X, Y - Q.Y, Z - Q.Z, W - Q.W); }
	/**
	...
	*/
	FORCEINLINE FQuat operator-=(const FQuat& Q) noexcept
	{
		this->X -= Q.X;
		this->Y -= Q.Y;
		this->Z -= Q.Z;
	...
	*/
	FORCEINLINE FQuat operator*(const float Scale) const noexcept { return FQuat(Scale * X, Scale * Y, Scale * Z, Scale * W); }
	/**
	...
	*/
	FORCEINLINE FQuat operator*(const FQuat& Q) const noexcept
	{
		FQuat Result;
	...
	*/
	FVector3D operator*(const FVector3D& V) const noexcept;
	/**
	...
	*/
	FMatrix operator*(const FMatrix& M) const noexcept;
	/**
	...
	*/
	FORCEINLINE FQuat operator*=(const float Scale) noexcept
	{
		X *= Scale;
		Y *= Scale;
		Z *= Scale;
	...
	*/
	FORCEINLINE float operator|(const FQuat& Q) const noexcept { return X * Q.X + Y * Q.Y + Z * Q.Z + W * Q.W; }
...
public:
	...
	*/
	FORCEINLINE bool operator==(const FQuat& Q) const noexcept { return X == Q.X && Y == Q.Y && Z == Q.Z && W == Q.W; }
	/**
...
public:
	...
	*/
	FORCEINLINE bool IsIdentity(float Tolerance = SMALL_NUMBER) const noexcept { return Equals(FQuat::Identity, Tolerance); }
	/**
	...
	*/
	FORCEINLINE bool Identical(const FQuat* Q, const uint32 PortFlags) const noexcept { return X == Q->X && Y == Q->Y && Z == Q->Z && W == Q->W; }
	...
	*/
	FORCEINLINE bool IsNormalized() const noexcept { return (FMath::Abs(1.f - SizeSquared()) < THRESH_QUAT_NORMALIZED); }
	...
	*/
	FORCEINLINE float Size() const noexcept { return FMath::Sqrt(X * X + Y * Y + Z * Z + W * W); }
	/**
	...
	*/
	FORCEINLINE FQuat Inverse() const noexcept { return FQuat(-X, -Y, -Z, W); }
	...
	*/
	FORCEINLINE float GetAngle() const noexcept { return 2.0f * FMath::Acos(W); }
	/**
...
public:
	...
	/**
		...
		@return The hypersphere-angle between two quaternions; alignment shouldn't matter, though.
	*/
	static FORCEINLINE float Error(const FQuat& Q1, const FQuat& Q2) noexcept
	{
		...
		return (FMath::Abs(cosom) < 0.9999999f) ? FMath::Acos(cosom) * (1.0f / PI) : 0.0f;
	}
	/**
	...
	*/
	...
	{
		return FQuat::FastLerp(FQuat::FastLerp(P00, P10, FracX), FQuat::FastLerp(P01, P11, FracX), FracY);
	}
	/** 
		Spherical interpolation. Will correct alignment. 
		Result is normalized.
	*/
	static FORCEINLINE FQuat Slerp(const FQuat& Quat1, const FQuat& Quat2, float Slerp) noexcept { return Slerp_NotNormalized(Quat1, Quat2, Slerp).GetNormalized(); }
	/**
		...
		Result is normalized.
	*/
	static FORCEINLINE FQuat SlerpFullPath(const FQuat& quat1, const FQuat& quat2, float Alpha) noexcept { return SlerpFullPath_NotNormalized(quat1, quat2, Alpha).GetNormalized(); }
	/** 
	...
	*/
	...
	/**
		...
		@param PrevP - The Quaternion at P-1.
		@param P - The Quaternion to return the tangent.
		@param NextP - The Quaternion P+1.
	...
	*/
	static FORCEINLINE FQuat FindBetween(const FVector3D& Vector1, const FVector3D& Vector2) noexcept { return FindBetweenVectors(Vector1, Vector2); }
	/**
