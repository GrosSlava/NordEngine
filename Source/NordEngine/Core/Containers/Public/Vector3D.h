...
public:
	...
	*/
	FORCEINLINE FVector3D(float InF) : X(InF), Y(InF), Z(InF) { }
	/**
	...
	*/
	FORCEINLINE FVector3D(float InX, float InY, float InZ) noexcept : X(InX), Y(InY), Z(InZ) { }
	FORCEINLINE FVector3D(const FVector3D& V) noexcept : X(V.X), Y(V.Y), Z(V.Z) { }
	/**
...
public:
	...
	*/
	FORCEINLINE FVector3D operator-(const FVector3D& V) const noexcept { return FVector3D(X - V.X, Y - V.Y, Z - V.Z); }
	/**
	...
	*/
	FORCEINLINE FVector3D operator-(float Bias) const noexcept { return FVector3D(X - Bias, Y - Bias, Z - Bias); }
	/**
		Get a negated copy of the vector.
	 
		@return A - negated copy of the vector.
	*/
	FORCEINLINE FVector3D operator-() const noexcept { return FVector3D(-X, -Y, -Z); }
	/**
	...
	*/
	FORCEINLINE FVector3D operator-=(const FVector3D& V) noexcept
	{
		X -= V.X;
		Y -= V.Y;
		Z -= V.Z;
	...
	*/
	FORCEINLINE FVector3D operator*(float Scale) const noexcept { return FVector3D(X * Scale, Y * Scale, Z * Scale); }
	/**
	...
	*/
	FORCEINLINE FVector3D operator*=(float Scale) noexcept
	{
		X *= Scale;
		Y *= Scale;
		Z *= Scale;
	...
	*/
	FORCEINLINE FVector3D operator/(float Scale) const
	{
	...
	/**
		...
		@return The result of division.
	*/
	FORCEINLINE FVector3D operator/(const FVector3D& V) const { return FVector3D(X / V.X, Y / V.Y, Z / V.Z); }
	/**
	...
	*/
	FORCEINLINE FVector3D operator^(const FVector3D& V) const noexcept { return FVector3D(Y * V.Z - Z * V.Y, Z * V.X - X * V.Z, X * V.Y - Y * V.X); }
	/**
	...
	*/
	FORCEINLINE float operator|(const FVector3D& V) const noexcept { return X * V.X + Y * V.Y + Z * V.Z; }
...
public:
	 ...
	 */
	FORCEINLINE bool operator==(const FVector3D& V) const noexcept { return X == V.X && Y == V.Y && Z == V.Z; }
	/**
	...
	*/
	FORCEINLINE bool operator<(const FVector3D& Other) const noexcept { return X < Other.X && Y < Other.Y && Z < Other.Z; }
	/**
...
public:
	...
	/**
		...
		@return The cross product.
	*/
	static FORCEINLINE FVector3D CrossProduct(const FVector3D& A, const FVector3D& B) noexcept { return A ^ B; }
	/**
		...
		@return The dot product.
	*/
	static FORCEINLINE float DotProduct(const FVector3D& A, const FVector3D& B) noexcept { return A | B; }
	/**
	...
	*/
	static FORCEINLINE float Triple(const FVector3D& X, const FVector3D& Y, const FVector3D& Z) noexcept { return ((X.X * (Y.Y * Z.Z - Y.Z * Z.Y)) + (X.Y * (Y.Z * Z.X - Y.X * Z.Z)) + (X.Z * (Y.X * Z.Y - Y.Y * Z.X))); }
	...
	*/
	static FORCEINLINE bool PointsAreNear(const FVector3D& Point1, const FVector3D& Point2, float Dist) noexcept
	{
		...
		if( FMath::Abs(Temp) >= Dist ) return false;
	...
	*/
	static FORCEINLINE float PointPlaneDist(const FVector3D& Point, const FVector3D& PlaneBase, const FVector3D& PlaneNormal) noexcept { return (Point - PlaneBase) | PlaneNormal; }
	/**
	...
	*/
	static FORCEINLINE FVector3D VectorPlaneProject(const FVector3D& V, const FVector3D& PlaneNormal) noexcept { return V - V.ProjectOnToNormal(PlaneNormal); }
	...
	*/
	static FORCEINLINE float Dist(const FVector3D& V1, const FVector3D& V2) noexcept { return FMath::Sqrt(FVector3D::DistSquared(V1, V2)); }
	/**
	...
	*/
	static FORCEINLINE float DistXY(const FVector3D& V1, const FVector3D& V2) noexcept { return FMath::Sqrt(FVector3D::DistSquaredXY(V1, V2)); }
	...
	*/
	static FORCEINLINE float DistSquared(const FVector3D& V1, const FVector3D& V2) noexcept { return FMath::Square(V2.X - V1.X) + FMath::Square(V2.Y - V1.Y) + FMath::Square(V2.Z - V1.Z); }
	/**
	...
	*/
	static FORCEINLINE float DistSquaredXY(const FVector3D& V1, const FVector3D& V2) noexcept { return FMath::Square(V2.X - V1.X) + FMath::Square(V2.Y - V1.Y); }
	...
	*/
	static FORCEINLINE float BoxPushOut(const FVector3D& Normal, const FVector3D& Size) noexcept { return FMath::Abs(Normal.X * Size.X) + FMath::Abs(Normal.Y * Size.Y) + FMath::Abs(Normal.Z * Size.Z); }
	...
	*/
	static FORCEINLINE bool Coplanar(const FVector3D& Base1, const FVector3D& Normal1, const FVector3D& Base2, const FVector3D& Normal2, float ParallelCosineThreshold = THRESH_NORMALS_ARE_PARALLEL) noexcept
	{
		...
		if( FMath::Abs(FVector3D::PointPlaneDist(Base2, Base1, Normal1)) > THRESH_POINT_ON_PLANE ) return false;
		return true;
	}
	...
	*/
	static FORCEINLINE FVector3D RadiansToDegrees(const FVector3D& RadVector) noexcept { return RadVector * (180.f / PI); }
	/**
	...
	*/
	static FORCEINLINE FVector3D DegreesToRadians(const FVector3D& DegVector) noexcept { return DegVector * (PI / 180.f); }
	...
	*/
	static FORCEINLINE FVector3D GetReflectionVector(const FVector3D& Direction, const FVector3D& SurfaceNormal)
	{
		const FVector3D SafeNormal(SurfaceNormal.GetSafeNormal());
...
public:
	...
	*/
	FORCEINLINE bool Equals(const FVector3D& V, float Tolerance = KINDA_SMALL_NUMBER) const noexcept { return FMath::Abs(X - V.X) <= Tolerance && FMath::Abs(Y - V.Y) <= Tolerance && FMath::Abs(Z - V.Z) <= Tolerance; }
	/**
		...
		@return true if the vectors are equal within tolerance limits, false otherwise.
	*/
	FORCEINLINE bool AllComponentsEqual(float Tolerance = KINDA_SMALL_NUMBER) const noexcept { return FMath::Abs(X - Y) <= Tolerance && FMath::Abs(X - Z) <= Tolerance && FMath::Abs(Y - Z) <= Tolerance; }
	...
	*/
	FORCEINLINE void SetComponentForAxis(EAxis Axis, float Component) noexcept
	{
		switch( Axis )
		{
		case EAxis::X: X = Component; break;
		case EAxis::Y: Y = Component; break;
		case EAxis::Z: Z = Component; break;
		}
	}
	/**
	...
	*/
	FORCEINLINE float GetMax() const noexcept { return FMath::Max(FMath::Max(X, Y), Z); }
	/**
		Get the maximum absolute value of the vector's components.
	 
		@return The maximum absolute value of the vector's components.
	*/
	FORCEINLINE float GetAbsMax() const noexcept { return FMath::Max(FMath::Max(FMath::Abs(X), FMath::Abs(Y)), FMath::Abs(Z)); }
	/**
		Get the minimum value of the vector's components.
	 
		@return The minimum value of the vector's components.
	*/
	FORCEINLINE float GetMin() const noexcept { return FMath::Min(FMath::Min(X, Y), Z); }
	/**
		Get the minimum absolute value of the vector's components.
	 
		@return The minimum absolute value of the vector's components.
	*/
	FORCEINLINE float GetAbsMin() const noexcept { return FMath::Min(FMath::Min(FMath::Abs(X), FMath::Abs(Y)), FMath::Abs(Z)); }
	/** 
		Gets the component-wise min of two vectors.
	*/
	FORCEINLINE FVector3D ComponentMin(const FVector3D& Other) const noexcept { return FVector3D(FMath::Min(X, Other.X), FMath::Min(Y, Other.Y), FMath::Min(Z, Other.Z)); }
	/** 
		Gets the component-wise max of two vectors.
	*/
	FORCEINLINE FVector3D ComponentMax(const FVector3D& Other) const noexcept { return FVector3D(FMath::Max(X, Other.X), FMath::Max(Y, Other.Y), FMath::Max(Z, Other.Z)); }
	...
	*/
	FORCEINLINE FVector3D GetAbs() const noexcept { return FVector3D(FMath::Abs(X), FMath::Abs(Y), FMath::Abs(Z)); }
	...
	*/
	FORCEINLINE float Size() const noexcept { return FMath::Sqrt(X * X + Y * Y + Z * Z); }
	/**
		Get the squared length of this vector.
	 
		@return The squared length of this vector.
	*/
	FORCEINLINE float SizeSquared() const noexcept { return X * X + Y * Y + Z * Z; }
	/**
		Get the length of the 2D components of this vector.
	 
		@return The 2D length of this vector.
	*/
	FORCEINLINE float Size2D() const noexcept { return FMath::Sqrt(X * X + Y * Y); }
	/**
		Get the squared length of the 2D components of this vector.
	 
		@return The squared 2D length of this vector.
	*/
	FORCEINLINE float SizeSquared2D() const noexcept { return X * X + Y * Y; }
	...
	*/
	FORCEINLINE bool IsNearlyZero(float Tolerance = KINDA_SMALL_NUMBER) const noexcept { return FMath::Abs(X) <= Tolerance && FMath::Abs(Y) <= Tolerance && FMath::Abs(Z) <= Tolerance; }
	/**
		Checks whether all components of the vector are exactly zero.
	 
		@return true if the vector is exactly zero, false otherwise.
	*/
	FORCEINLINE bool IsZero() const noexcept { return X == 0.f && Y == 0.f && Z == 0.f; }
	...
	*/
	FORCEINLINE bool IsUnit(float LengthSquaredTolerance = KINDA_SMALL_NUMBER) const noexcept { return FMath::Abs(1.0f - SizeSquared()) < LengthSquaredTolerance; }
	...
	*/
	FORCEINLINE bool IsNormalized() const noexcept { return (FMath::Abs(1.f - SizeSquared()) < THRESH_VECTOR_NORMALIZED); }
	...
	*/
	FORCEINLINE FVector3D GetSignVector() const noexcept { return FVector3D(FMath::FloatSelect(X, 1.f, -1.f), FMath::FloatSelect(Y, 1.f, -1.f), FMath::FloatSelect(Z, 1.f, -1.f)); }
	...
	*/
	FORCEINLINE FVector3D GridSnap(const float& GridSz) const noexcept { return FVector3D(FMath::GridSnap(X, GridSz), FMath::GridSnap(Y, GridSz), FMath::GridSnap(Z, GridSz)); }
	/**
	...
	*/
	FORCEINLINE FVector3D BoundToCube(float Radius) const noexcept { return FVector3D(FMath::Clamp(X, -Radius, Radius), FMath::Clamp(Y, -Radius, Radius), FMath::Clamp(Z, -Radius, Radius)); }
	/** 
		Get a copy of this vector, clamped inside of a cube. 
	*/
	FORCEINLINE FVector3D BoundToBox(const FVector3D& Min, const FVector3D Max) const noexcept { return FVector3D(FMath::Clamp(X, Min.X, Max.X), FMath::Clamp(Y, Min.Y, Max.Y), FMath::Clamp(Z, Min.Z, Max.Z)); }
	/**
		...
		@param Radius Half size of the cube.
	*/
	FORCEINLINE void AddBounded(const FVector3D& V, float Radius = MAX_int16) noexcept { *this = (*this + V).BoundToCube(Radius); }
	...
	*/
	FORCEINLINE bool IsUniform(float Tolerance = KINDA_SMALL_NUMBER) const noexcept { return AllComponentsEqual(Tolerance); }
	...
	*/
	FORCEINLINE FVector3D MirrorByVector(const FVector3D& MirrorNormal) const noexcept { return *this - MirrorNormal * (2.f * (*this | MirrorNormal)); }
	/**
	...
	*/
	FORCEINLINE FVector3D ProjectOnTo(const FVector3D& A) const noexcept { return (A * ((*this | A) / (A | A))); }
	/**
		...
		@return Projected vector.
	*/
	FORCEINLINE FVector3D ProjectOnToNormal(const FVector3D& Normal) const noexcept { return (Normal * (*this | Normal)); }
	...
	*/
	FORCEINLINE bool ContainsNaN() const noexcept { return (!FMath::IsFinite(X) || !FMath::IsFinite(Y) || !FMath::IsFinite(Z)); }
...
*/
FORCEINLINE FVector3D operator*(float Scale, const FVector3D& V) noexcept
{
