...
public:
	 ...
	 */
	FORCEINLINE FRotationTranslationMatrix(const FRotator& Rot, const FVector3D& Origin) noexcept
	{
...
public:
	...
	static FMatrix Make(const FRotator& Rot, const FVector3D& Origin)
	{
...
public:
	 ...
	 * @param Rot rotation
	 */
	FORCEINLINE FRotationMatrix(const FRotator& Rot) :
		FRotationTranslationMatrix(Rot, FVector3D::ZeroVector)
	{ 
...
public:
	...
	/** Matrix factory. Return an FMatrix so we don't have type conversion issues in expressions. */
	static FMatrix Make(FRotator const& Rot) noexcept
	{
		return FRotationMatrix(Rot);
	}

	/** Matrix factory. Return an FMatrix so we don't have type conversion issues in expressions. */
	static FMatrix Make(FQuat const& Rot) noexcept
	{
		return FQuatRotationTranslationMatrix(Rot, FVector3D::ZeroVector);
	}
