...
public:
	 ...
	 */
	explicit FORCEINLINE FVector4D(float InX = 0.0f, float InY = 0.0f, float InZ = 0.0f, float InW = 1.0f) noexcept : 
...
public:
	 ...
	 */
	FORCEINLINE FVector4D operator-() const noexcept
	{
	 ...
	 */
	FORCEINLINE FVector4D operator+(const FVector4D& V) const noexcept
	{
		return FVector4D(X + V.X, Y + V.Y, Z + V.Z, W + V.W);
	}
	 ...
	 */
	FORCEINLINE FVector4D operator*(float Scale) const noexcept
	{
		return FVector4D(X * Scale, Y * Scale, Z * Scale, W * Scale);
	}
	 ...
	 */
	friend FORCEINLINE FVector4D operator*(float Scale, const FVector4D& V) noexcept
	{
...
public:
	 ...
	 */
	FORCEINLINE bool operator==(const FVector4D& V) const
	{
...
public:
	 ...
	 */
	friend FORCEINLINE float Dot3(const FVector4D& V1, const FVector4D& V2)
	{
	 ...
	 */
	friend FORCEINLINE float Dot4(const FVector4D& V1, const FVector4D& V2)
	{
	 ...
	 */
	FORCEINLINE bool IsUnit3(float LengthSquaredTolerance = KINDA_SMALL_NUMBER) const noexcept
	{
	 ...
	 */
	FORCEINLINE float Size3() const noexcept
	{
	 ...
	 */
	FORCEINLINE float SizeSquared3() const noexcept
	{
		return X * X + Y * Y + Z * Z;
	}
	...
	FORCEINLINE bool IsNearlyZero3(float Tolerance = KINDA_SMALL_NUMBER) const noexcept
	{
		return
	...
	/** Reflect vector. */
	FORCEINLINE FVector4D Reflect3(const FVector4D& Normal) const noexcept
	{
	...
	FORCEINLINE void FindBestAxisVectors3(FVector4D& Axis1, FVector4D& Axis2) const noexcept
	{
		...
		if (NZ > NX && NZ > NY)	Axis1 = FVector4D(1, 0, 0);
		else					Axis1 = FVector4D(0, 0, 1);
