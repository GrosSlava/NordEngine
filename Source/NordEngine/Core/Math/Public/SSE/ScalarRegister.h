...
*/
class ENGINE_API ScalarRegister
{
public:

	FORCEINLINE ScalarRegister() noexcept { }
	FORCEINLINE ScalarRegister(const ScalarRegister& VectorValue) noexcept { Value = VectorValue.Value; }
	explicit FORCEINLINE ScalarRegister(const float& ScalarValue) noexcept { Value = VectorLoadFloat1(&ScalarValue); }
	explicit FORCEINLINE ScalarRegister(const VectorRegister& VectorValue) noexcept { Value = VectorValue; }
...
public:
	...
	*/
	FORCEINLINE ScalarRegister operator*(const ScalarRegister& RHS) const noexcept { return ScalarRegister(VectorMultiply(Value, RHS.Value)); }
	/**
	...
	*/
	FORCEINLINE ScalarRegister operator+(const ScalarRegister& RHS) const noexcept { return ScalarRegister(VectorAdd(Value, RHS.Value)); }
	/**
	...
	*/
	FORCEINLINE ScalarRegister operator-(const ScalarRegister& RHS) const noexcept { return ScalarRegister(VectorSubtract(Value, RHS.Value)); }
	/**
		Assignment operator.
	...
	*/
	FORCEINLINE operator VectorRegister() const noexcept { return Value; }
